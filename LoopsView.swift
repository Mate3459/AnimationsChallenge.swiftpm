...
//  AnimationsChallenge
//
//  Created by Brock R. Wrede on 9/29/23.
//
...
struct LoopsView: View {
    ...
    func stretch1ButtonPressed() {
        ...
        for quinn in values {
            if quinn == number5 {
                break}
            numberset = numberset + quinn + ","}
        finalnumberset = numberset + number5
        output = finalnumberset}
    func stretch2ButtonPressed() {
        setValues()
        var maximum = Int(values[0])
        var i = 0
        while i < values.count{
            if Int(values[i])! > maximum! {
                maximum = Int(values[i])!}
            i = i + 1}
    ...
    func stretch4ButtonPressed() {
        ...
        var i = 0
        for num in values{
            if Int(num)! <= Int(exactly: minimum)!{
                minimum = Int(num)!}
        }
    while i < values.count{
        if Int(values[i])! == Int(exactly: minimum) {
            index = Int(exactly: i)!}
        i = i + 1}
    ...
    func stretch5ButtonPressed() {
        setValues()
        // MARK: - Stretch #5
        for printble in namesAndScores {
            output = output + "\(printble.key): \(printble.value)\n"}
    }
}

struct BrocksPreview: PreviewProvider {
